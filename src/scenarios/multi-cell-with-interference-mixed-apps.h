...
static int GetCommonMultiSeed (int i)
{
  if (i >= 0 && i< 9 )
	  return commomMultiSeed[i];
  else
	  return commomMultiSeed[0];
}

static void MultiCellWithInterferenceMixedApps (int nbCells, double radius,
                                        int nbUE,
                                        ...
                                        int seed)
{
	...
	double bandwidth = 5;
  	...
  	// CONFIGURE SEED
  	if (seed >= 0){
  		int commonSeed = GetCommonMultiSeed (seed);
	  	srand (commonSeed);
	} else {
	  	srand (0);
	}
	...
	//Define Application Container
  	TraceBased VideoApplication[nbVideo*nbCells];
	InfiniteBuffer BEApplication[nbBE*nbCells];
	VoIP VoIPApplication[nbVoIP*nbCells];
  	CBR CBRApplication[nbCBR*nbCells];
