...
float before_tti_gbr=0;
float before_tti_goodput=0;
float before_tti_delay=0;
float before_tti_plr=0;
float before_tti_fairness=0;
...
h_log("debug3003\n");
		...
		torch::Tensor CalculateReward(){
			...
			int sum_count=0;
			float calculate_gbr=0;
			float calculate_goodput=0;
			float calculate_delay=0;
			float calculate_plr=0;
	      	...
	      	for (std::vector<UESummary*>::iterator it = GetUESummaryContainer()->begin(); it != GetUESummaryContainer()->end(); ++it){
				i++;
	         	for (std::vector<Application*>::iterator itt = (*it)->GetApplicationContainer()->begin(); itt != (*it)->GetApplicationContainer()->end(); ++itt){
					...
					calculate_gbr+=(*(*itt)).realgbr;
					calculate_goodput+=(*(*itt)).realgoodput;
					calculate_delay+=(*(*itt)).realdelay;
					calculate_plr+=(*(*itt)).realplr;
					sum_count++;

					// * gbr
					if ((*(*itt)).realgbr > 2400 ) gbrReward = 2;
					else if ((*(*itt)).realgbr > 2200 ) gbrReward = 1;					
					else if ( TTIcounter>15000 && (*(*itt)).realgbr > before_gbr[i][0] + 0.5 ) gbrReward = 1;
					
					//else if ((*(*itt)).realgbr > 2100 ) gbrReward = 0.7;
					//else if ((*(*itt)).realgbr > 2000 ) gbrReward = 0.5;
					else if ((*(*itt)).realgbr > 2100 ) gbrReward = 0.5;
					else if ((*(*itt)).realgbr > 2000 ) gbrReward = 0;
					//else if ((*(*itt)).realgbr > 1700 ) gbrReward = -0.3;
					else if ((*(*itt)).realgbr > 1800 ) gbrReward = -0.5;
					...
					else if (TTIcounter>15000 && (*(*itt)).realgoodput > before_goodput[i][0] + 10 ) goodputReward = 1;
					...
					else if (TTIcounter>15000 && (*(*itt)).realdelay < before_delay[i][0] - 0.02 ) delayReward = 1;
					...
					else if (TTIcounter>15000 && (*(*itt)).realplr < before_plr[i][0] + 0.0002) plrReward = 1;
			...
			//* calculate reward start
			calculate_gbr = calculate_gbr / sum_count;
			...
			else if (calculate_delay < 0.120 ) delayReward = -1;
			else delayReward = -1;
			before_tti_delay = calculate_delay;

			// * PLR
			if (calculate_plr < 0.68 ) plrReward = 3;
			else if (calculate_plr < 0.69 ) plrReward = 1;
			else if (TTIcounter>15000 && calculate_plr < before_tti_plr - 0.00002) plrReward = 0.5;
			else if (calculate_plr < 0.70 ) plrReward = 0.5;
			//else if ((*(*itt)).realplr < 0.70 ) plrReward = 0.3;
			else if (calculate_plr < 0.71 ) plrReward = 0;
			//else if ((*(*itt)).realplr < 0.72 ) plrReward = -0.3;
			else if (calculate_plr < 0.72 ) plrReward = -0.5;
			else if (calculate_plr < 0.73 ) plrReward = -1;
			else plrReward = -1;
			...
			else if (jfi >= 0.58 ) fairness_reward = 0;
			else if (jfi >= 0.55 ) fairness_reward = -0.5;
			else if (jfi >= 0.53 ) fairness_reward = -1;
			...
			//	calculate_gbr,gbrReward,calculate_delay,delayReward,calculate_plr,plrReward,jfi,fairness_reward, sum_reward);
